...
### Step 1
We will start by installing a FHIR server and loading in test data. An IRIS for Health server has been made available on "http://g1.isc-ohdsiworkshop.com/irishealth/csp/sys/UtilHome.csp" where you should replace your g1 accordingly. Log in using username OHDSIUSER and password ohdsi1009. You are now logged in on the management portal of IRIS for Health. Make sure you are in the OHDSI namespace. If not, on the top click on the namespace specified next to namespace in order to switch.
...
### Step 1
First, configure the credentials that the Bulk FHIR Coordinator uses to connect to the FHIR Server. In order to do so, go to management portal again (http://g1.isc-ohdsiworkshop.com/irishealth/csp/sys/UtilHome.csp). Go to Interoperability <- Configure <- Credentials. There, create a new credential with ID OHDSICredentials and your username and password (OHDSIUSER and ohdsi1009). Save this credential.
...
Under Fetch type, select HS.BulkFHIR.Fetch.PureFHIR.Adapter. Select the Endpoint url to be the same as the one for the FHIR server you created in Part 2 (http://g1.isc-ohdsiworkshop.com/irishealth/csp/healthshare/ohdsi/fhir/r4). Set the Authorization type to be HTTP, allowing to connect to the FHIR server using Basic Authentication. Select the HTTP credentials you created in step 1.
...
Now, under Processes, click on ToS3Upload. Then, on the right (in the Settings tab for this component), look for the BucketSubfolder setting under Basic Settings. Set this to be Transaction/in/g1/ where group_id should be replaced accordingly (example Transaction/in/g1/). Then click apply. Please note the final slash is required. Under informational settings, you can click on the looking glass next to the Class Name. This will open the BPL code (a graphical editor) for this business process.
...
- Setting Value: http://g1.isc-ohdsiworkshop.com/irishealth/csp/healthshare/ohdsi/fhir/r4 (the full FHIR URL)

Now go back to the Production. Click on the RESTBulkFHIR operation and configure it using the following settings:
- HTTP Server: g1.isc-ohdsiworkshop.com
- HTTP Port: 80
...
```
Confirm these are the ones as expected by executing GET http://g1.isc-ohdsiworkshop.com/irishealth/csp/healthshare/ohdsi/fhir/r4/Patient?gender=female&birthdate=lt2006-09-07 in Postman (or you API clinet).
